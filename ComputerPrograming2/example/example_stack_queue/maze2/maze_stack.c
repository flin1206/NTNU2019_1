...
sMazeStack *alloc_maze( int32_t row, int32_t column, int32_t direction )
{
    sMazeStack  *pNewOne = calloc( 1, sizeof( sMazeStack ) );
    
    pNewOne -> pos_row = row;
    pNewOne -> pos_column = column;
    pNewOne -> direction = direction;
...
void print_maze_path( void )
{
    ...
    list_for_each_prev( listptr, &maze_stack )
    {
        sMazeStack *pStack = list_entry( listptr, sMazeStack, list );
        printf( "(%2d, %2d) -> ", pStack -> pos_row, pStack -> pos_column );
    }
    return;
}
